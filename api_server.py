...
# === CLEAN URL (remove playlists, radios, tracking) ===
...
def api_info():
    ...
    try:
        ...
        # Increased timeout to 180 s for Render slow CPUs
...
def api_download():
    ...
    try:
        # Get title first (longer timeout)
        info_cmd = [
            "yt-dlp", "--no-warnings", "--skip-download",
            "--no-check-certificates", "--cookies", "cookies.txt", "-j", url
        ]
        ...
        return jsonify({"error": "Timeout while downloading video"}), 504
    except Exception as e:
        return jsonify({"error": str(e)}), 500
